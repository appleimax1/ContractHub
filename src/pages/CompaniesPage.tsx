...
import db, { type MyCompany, type BankAccount } from '../db/db';
...
export default function CompaniesPage() {
  ...
  const defaultForm: Partial<MyCompany> = {
    name: '',
    bin_iin: '',
    address_legal: '',
    address_actual: '',
  ...
  const [formData, setFormData] = useState<Partial<MyCompany>>(defaultForm);

  const openForm = (company?: MyCompany) => {
    if (company) {
      setFormData(company);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingId) {
        await db.my_companies.update(editingId, formData as any);
        toast.success('Компания успешно обновлена');
      } else {
        await db.my_companies.add(formData as MyCompany);
  ...
  const handleDelete = async (id: number) => {
    if (confirm('Удалить компанию?')) {
      await db.my_companies.delete(id);
    }
  };
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>, field: 'stamp_image' | 'signature_image') => {
  ...
  const removeImage = (field: 'stamp_image' | 'signature_image') => {
    setFormData(prev => ({
      ...prev,
      [field]: ''
    }));
  ...
  return (
    <div className="p-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <h1 className="text-2xl font-bold text-gray-800">Мои Компании</h1>
      ...
      {isEditing && (
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 mb-6">
          <h2 className="text-xl font-semibold mb-6 pb-2 border-b">{editingId ? 'Редактировать компанию' : 'Новая компания'}</h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="space-y-4">
              ...
              <h3 className="font-medium text-blue-700 pt-4">Печать и Подпись</h3>
              <div className="grid grid-cols-2 gap-4">
                <div className="border border-gray-300 rounded-md p-4 flex flex-col items-center justify-center relative bg-gray-50 min-h-[120px]">
                  {formData.stamp_image ? (
                    <>
                      <img src={formData.stamp_image} alt="Печать" className="max-h-24 object-contain" />
                      <button onClick={() => removeImage('stamp_image')} className="absolute top-2 right-2 text-red-500 hover:text-red-700 bg-white rounded-full p-1 shadow">
                        <Trash2 size={14} />
                      </button>
                    </>
                  ) : (
                    <label className="cursor-pointer text-center flex flex-col items-center text-gray-500 hover:text-blue-600">
                      <span className="text-sm font-medium">Загрузить печать</span>
                      <span className="text-xs mt-1">(PNG без фона)</span>
                      <input type="file" accept="image/png, image/svg+xml" className="hidden" onChange={e => handleImageUpload(e, 'stamp_image')} />
                    </label>
                  )}
                </div>
                <div className="border border-gray-300 rounded-md p-4 flex flex-col items-center justify-center relative bg-gray-50 min-h-[120px]">
                  {formData.signature_image ? (
                    <>
                      <img src={formData.signature_image} alt="Подпись" className="max-h-24 object-contain" />
                      <button onClick={() => removeImage('signature_image')} className="absolute top-2 right-2 text-red-500 hover:text-red-700 bg-white rounded-full p-1 shadow">
                        <Trash2 size={14} />
                      </button>
                    </>
                  ) : (
                    <label className="cursor-pointer text-center flex flex-col items-center text-gray-500 hover:text-blue-600">
                      <span className="text-sm font-medium">Загрузить подпись</span>
                      <span className="text-xs mt-1">(PNG без фона)</span>
                      <input type="file" accept="image/png, image/svg+xml" className="hidden" onChange={e => handleImageUpload(e, 'signature_image')} />
                    </label>
                  )}
                </div>
              </div>
      ...
      {!isEditing && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {companies?.filter(c => 
            ...
            <div key={company.id} className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 flex flex-col">
              <div className="flex justify-between items-start mb-4 border-b pb-4">
                <div>
                  <h3 className="text-xl font-semibold text-gray-800">{company.name}</h3>
                  <p className="text-sm text-gray-500">БИН/ИИН: {company.bin_iin}</p>
                </div>
                <div className="flex space-x-2">
                  <button onClick={() => openForm(company)} className="text-blue-500 hover:text-blue-700 p-1">
                    <Edit size={18} />
                  </button>
                  <button onClick={() => handleDelete(company.id!)} className="text-red-500 hover:text-red-700 p-1">
                    <Trash2 size={18} />
                  </button>
                </div>
              </div>
              <div className="space-y-2 text-sm text-gray-600 flex-1">
                <p><strong>Руководитель:</strong> {company.ceo_title} {company.ceo_name}</p>
                <p><strong>Email:</strong> {company.email || '-'}</p>
                <p><strong>Счетов:</strong> {company.bank_accounts?.length || 0}</p>
              </div>
            </div>
          ))}
          {companies?.filter(c => 
            ...
            <div className="col-span-full text-center py-12 text-gray-500 bg-white rounded-lg border border-gray-200">
              {searchQuery ? 'Компании по вашему запросу не найдены.' : 'У вас пока нет ни одной компании. Добавьте первую!'}
            </div>
          )}
        </div>
      )}
